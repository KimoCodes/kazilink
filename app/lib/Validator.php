...
final class Validator
{
    public static function trim(array $input): array
    {
        ...
        foreach ($input as $key => $value) {
            if (is_string($value)) {
                ...
                $trimmed[$key] = normalize_whitespace(trim($value));
            } elseif (is_array($value)) {
                $trimmed[$key] = self::trim($value);
            } else {
                $trimmed[$key] = $value;
            }
        }
    ...
    public static function sanitizeNumeric(string $input, float $min = 0, float $max = PHP_FLOAT_MAX): ?float
    {
        $numeric = filter_var($input, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION | FILTER_FLAG_ALLOW_THOUSAND);
