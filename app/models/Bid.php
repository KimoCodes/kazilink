...
final class Bid
{
    ...
    public function forClientTask(int $taskId, int $clientId): array
    {
        $statement = $this->db->prepare('
            ...
            LEFT JOIN subscriptions s ON s.user_id = b.tasker_id AND s.id = (SELECT MAX(id) FROM subscriptions WHERE user_id = b.tasker_id)
            LEFT JOIN plans tasker_plan ON tasker_plan.id = COALESCE(s.active_plan_id, s.plan_id)
