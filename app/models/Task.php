...
final class Task
{
    ...
    public function findOpenById(int $id, ?array $viewerPlan = null): ?array
    {
        ...
        if ($viewerPlan !== null) {
            ...
            $visibilityClause = ' AND t.created_at <= DATE_SUB(NOW(), INTERVAL ' . $delayMinutes . ' MINUTE)';
        }
    ...
    private function browseOpenBaseQuery(array $filters, ?array $viewerPlan = null, bool $countOnly = false): array
    {
        ...
        $select = $countOnly
            ...
            : '
                ...
                COALESCE(plan_visibility.plan_name, "Basic Trial") AS client_plan_name,
        ...
        if (!$countOnly) {
            ...
            ) AS plan_visibility ON plan_visibility.user_id = t.client_id
            ';
        }
