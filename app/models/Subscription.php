...
final class Subscription
{
    ...
    public function markLatestPaymentPendingForUser(int $userId, int $planId, string $momoReference): void
    {
        if (!Database::tableExists('subscriptions')) {
            throw new RuntimeException('The subscriptions table does not exist yet. Run the subscription migration first.');
        }
        ...
        if ($subscription === false) {
            throw new RuntimeException('Unable to initialize a subscription record for the pending payment.');
        }

        $this->markPaymentPending((int) $subscription['id'], $planId, $momoReference);
    }

    public function activateFromSuccessfulPayment(int $userId, int $planId, string $momoReference): void
    {
        $this->activateFromVerifiedPayment($userId, $planId, $momoReference);
    }

    public function activateFromVerifiedPayment(int $userId, int $planId, string $paymentReference, ?string $activationAt = null): void
    {
        if (!Database::tableExists('subscriptions')) {
            throw new RuntimeException('The subscriptions table does not exist yet. Run the subscription migration first.');
        }
        ...
        try {
            ...
            if ((string) ($subscription['momo_reference'] ?? '') === $paymentReference && (string) $subscription['status'] === 'active') {
                $this->db->commit();

                return;
            }

            if ((string) ($subscription['momo_reference'] ?? '') !== $paymentReference) {
            ...
            foreach ([
                ...
                try {
                    $candidateDate = new DateTimeImmutable($candidate, $timezone);

                    if ($candidateDate > $anchor) {
                        $anchor = $candidateDate;
                    }
                } catch (Throwable) {
                    continue;
                }
            ...
            $periodEndsAt = $anchor->modify('+1 month')->format('Y-m-d H:i:s');
