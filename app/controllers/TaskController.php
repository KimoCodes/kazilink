...
final class TaskController
{
    ...
    public function edit(): string
    {
        ...
        // Resolve ID consistently. If POSTed, we prioritize the ID in the body to ensure we check the actually intended record.
        ...
        // Initial fetch for both display and POST verification
        ...
        if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
            ...
            // Explicit re-check of ownership just before mutation to prevent race conditions or ID switching
            $finalTaskCheck = $this->tasks->findByIdForClient($taskId, (int) Auth::id());
            if ($finalTaskCheck === null) {
                Session::flash('error', 'You do not have permission to modify this task.');
                redirect('tasks/index');
            }
            ...
            if (!$success) {
                Session::flash('error', 'Failed to update task. It might have been modified by another process.');
                redirect('tasks/show', ['id' => $taskId]);
            }
    ...
    public function cancel(): string
    {
        ...
        $success = $this->tasks->cancel($taskId, (int) Auth::id());

        if (!$success) {
            Session::flash('error', 'Failed to cancel task. It might have been already booked or closed.');
            redirect('tasks/show', ['id' => $taskId]);
        }
