...
final class AdminController
{
    ...
    public function replyMessage(): string
    {
        ...
        $sent = LeadCapture::deliverAdminReply([
            'to_email' => $toEmail,
            'subject' => $subject,
            'message' => $body,
        ...
        $replyPayload = [
            ...
            'admin_user_id' => (int) Auth::id(),
            'admin_name' => (string) ($currentAdmin['full_name'] ?? $currentAdmin['email'] ?? 'Admin'),
            'delivery_status' => $sent ? 'sent' : 'logged_only',
        ];

        $saved = LeadCapture::append('contact-replies', $replyPayload);

        Session::flash(($saved || $sent) ? 'success' : 'warning', ($saved || $sent)
            ? 'Reply recorded' . ($sent ? ' and sent to the contact.' : ' locally. Email delivery is not configured on this machine.')
