...
try {
    ...
    if (($siteThemeValues['mode'] ?? 'light') === 'dark') {
        ...
        $siteThemeCss[] = '    /* Dark Mode Core Colors */';
        $siteThemeCss[] = '    --color-bg: #0f172a;';
        $siteThemeCss[] = '    --color-surface: #1e293b;';
        $siteThemeCss[] = '    --color-surface-muted: #334155;';
        $siteThemeCss[] = '    --color-surface-strong: #475569;';
        $siteThemeCss[] = '    --color-border: #334155;';
        $siteThemeCss[] = '    --color-border-strong: #64748b;';
        $siteThemeCss[] = '    --color-text: #f8fafc;';
        $siteThemeCss[] = '    --color-text-soft: #cbd5e1;';
        $siteThemeCss[] = '    --color-text-faint: #94a3b8;';
        $siteThemeCss[] = '    --color-text-muted: var(--color-text-soft);';
        $siteThemeCss[] = '';
        $siteThemeCss[] = '    /* Dark Mode Accents */';
        $siteThemeCss[] = '    --color-primary: #14b8a6;';
        $siteThemeCss[] = '    --color-primary-strong: #0d9488;';
        $siteThemeCss[] = '    --color-primary-soft: #0f766e;';
        $siteThemeCss[] = '    --color-secondary: #fbbf24;';
        $siteThemeCss[] = '    --color-secondary-strong: #f59e0b;';
        $siteThemeCss[] = '';
        $siteThemeCss[] = '    /* Dark Mode Semantic Colors */';
        $siteThemeCss[] = '    --color-success: #10b981;';
        $siteThemeCss[] = '    --color-success-soft: rgba(35, 90, 58, 0.45);';
        $siteThemeCss[] = '    --color-warning: #f59e0b;';
        $siteThemeCss[] = '    --color-warning-soft: rgba(104, 72, 31, 0.4);';
        $siteThemeCss[] = '    --color-danger: #ef4444;';
        $siteThemeCss[] = '    --color-danger-soft: rgba(118, 51, 55, 0.42);';
        $siteThemeCss[] = '    --color-info: #3b82f6;';
        $siteThemeCss[] = '    --color-info-soft: rgba(43, 84, 118, 0.38);';
        $siteThemeCss[] = '';
        $siteThemeCss[] = '    /* Dark Mode Shadows */';
        $siteThemeCss[] = '    --color-shadow: 0 28px 80px rgba(0, 0, 0, 0.22);';
        $siteThemeCss[] = '    --color-shadow-soft: 0 4px 6px -1px rgba(0, 0, 0, 0.3), 0 2px 4px -2px rgba(0, 0, 0, 0.2);';
        $siteThemeCss[] = '    --focus-ring: 0 0 0 4px rgba(20, 184, 166, 0.3);';
        $siteThemeCss[] = '}';
    } else {
...
<head>
    ...
    <?php else: ?>
        <link href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&family=Outfit:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <?php endif; ?>
...
$notifications = ['messages' => 0, 'bids' => 0, 'alerts' => 0];
...
if ($currentUser !== null) {
    ...
    $notifications['alerts'] = $notificationModel->unreadCount($role === 'admin' ? 'admin' : 'user', (int) Auth::id());
...
if (Auth::check()) {
    if (Auth::role() === 'client') {
        ...
        $workspaceLinks[] = ['route' => 'bookings/index', 'label' => 'My Bookings'];
        ...
        $workspaceLinks[] = ['route' => 'notifications/*', 'label' => 'Alerts', 'badge' => $notifications['alerts'], 'href' => url_for('notifications/index')];
    ...
    if (Auth::role() === 'admin') {
        ...
        $workspaceLinks[] = ['route' => 'bookings/index', 'label' => 'Bookings'];
        $workspaceLinks[] = ['route' => 'notifications/*', 'label' => 'Alerts', 'badge' => $notifications['alerts'], 'href' => url_for('notifications/index')];
        $workspaceLinks[] = ['route' => 'admin/messages', 'label' => 'Messages'];
        $workspaceLinks[] = ['route' => 'admin/newsletter', 'label' => 'Subscribers'];
        $workspaceLinks[] = ['route' => 'admin/newsletter-campaigns', 'label' => 'Campaigns'];
        $workspaceLinks[] = ['route' => 'admin/email-delivery', 'label' => 'Email'];
    }
}
?>
<body
